...
class ScheduleUtil {
  ...
  static async splitSemesterSchedule(semesterSchedule) {
    ...
    for (let i = 0; i < weeks; ++i) {
      ...
      const before = ele => ele.date <= weekEndStr;
      const after = ele => ele.date > weekEndStr;
  ...
  static async getSchedule(username, password, isUndergraduate, currentWeek) {
    ...
    const scheduleUndergraduateArgs = `?m=bks_jxrl_all&p_start_date=${startDate}&p_end_date=${endDate}`
          + '\&jsoncallback=no_such_method';
    const scheduleGraduateArgs = `?m=yjs_jxrl_all&p_start_date=${startDate}&p_end_date=${endDate}`
          + '\&jsoncallback=no_such_method';
