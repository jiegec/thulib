...
class LearnHelperUtil {
  ...
  async getCourseList() {
    try {
      // previous semester 2, current semester 1 and next semester 7
      let ps = [];
      for (const typepage of [1, 2, 7]) {
        const $ = await rp({
          method: 'GET',
        ...
        ps = ps.concat(Array.from($('.info_tr, .info_tr2')).map(tr => new Promise(async (resolve) => {
          ...
          if (url.indexOf('learn.cic.tsinghua.edu.cn') !== -1) {
            ...
            const isLearn2018 = await rp({
              ...
              jar: this.cookies,
              transform: (body, res) => body.indexOf('learn2018.tsinghua.edu.cn') !== -1,
            });
            if (isLearn2018) {
            ...
            course.teacher = _$('#table_box .tr_1').eq(4).text().replace(/&nbsp;/gi, '')
              .trim();
            course.email = _$('#table_box .tr_1').eq(5).text().replace(/&nbsp;/gi, '')
              .trim();
            course.phone = _$('#table_box .tr2').eq(4).text().replace(/&nbsp;/gi, '')
              .trim();
    ...
    try {
      ...
      const ps = Array.from($('.tr1, .tr2')).map(tr => new Promise(async (resolve) => {
        ...
        if (oldAssignment) {
          ...
          assignment.detail = $1('#table_box .tr_2').eq(1).text().replace(/&nbsp;/gi, '')
            .trim();
          assignment.filename = $1('#table_box .tr_2').eq(2).text().replace(/&nbsp;/gi, '')
            .trim();
          ...
          assignment.uploadFilename = $1('#table_box .tr_2').eq(4).text().replace(/&nbsp;/gi, '')
            .trim();
          assignment.uploadFileURL = $1('#table_box .tr_2').eq(4).find('a').attr('href');
          assignment.uploadFileURL = !assignment.uploadFileURL ? '' : this.prefix + assignment.uploadFileURL;
          ...
          assignment.evaluatingTeacher = $2('#table_box .tr_12').eq(0).text().replace(/&nbsp;/gi, '')
            .trim();
          let evaluatingDate = $2('#table_box .tr_12').eq(1).text().replace(/&nbsp;/gi, '')
            .trim();
          ...
          assignment.grade = parseFloat($2('#table_box .tr_1').eq(2).text().replace('分', '0')
            .trim());
          assignment.grade = isNaN(assignment.grade) ? -1 : assignment.grade;
          assignment.comment = $2('#table_box .tr_12').eq(2).text().replace(/&nbsp;/gi, '')
            .trim();
    ...
    try {
      ...
      const ps = Array.from($('.tr1, .tr2')).map(tr => new Promise(async (resolve) => {
        ...
        if (oldNotice) {
          ...
          const content_tr = $notice('.tr_l2').eq(1);
