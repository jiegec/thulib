...
class CurriculumUtil {
  ...
  static async parseFirstLevelCurriculum($) {
    ...
    $('.kc_div').each(async (i, elem) => {
      ...
      // Course Name
      const courseName = $(elem).find('h5').first().children()
        .first()
        .text();
      // Teacher
      const teacherStr = $(elem).find('h5').first().children()
        .last()
        .text();
  ...
  static async getFirstLevelCurriculum(username, password, isUndergraduate) {
    ...
    const curriculumGraduateFirstLevelUrlMobile = `${prefix}/portal3rd.do?m=yjs_kbSearch&mobile=true`;
