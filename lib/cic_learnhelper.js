...
class CicLearnHelperUtil {
  constructor(user) {
    ...
    this.prefix = 'https://learn.cic.tsinghua.edu.cn';
  }
  ...
  async getNotices(courseID) {
    const noticeUrl = `${this.prefix}/b/myCourse/notice/listForStudent/${courseID}?currentPage=1&pageSize=1000&_=${new Date().getTime()}`;
    try {
      const res = await rp({
        method: 'GET',
        uri: noticeUrl,
  ...
  async getDocuments(courseID) {
    ...
    try {
      ...
      const items = [];
      list.map((obj) => {
        items.push(...obj.courseCoursewareList);
      });
